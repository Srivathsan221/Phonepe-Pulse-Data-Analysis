...
video_file1=open('videos/phonepesample.mp4','rb')
...
# Creating Options in Application
SELECT = option_menu(
        menu_title = None,
...
if SELECT == "Basic insights":
    ...
    #st.write("----")
    st.subheader("Basic insights about data!")
...
if SELECT == "Home":
    st.subheader(
        "PhonePe is an Indian digital payments and fintech company headquartered in Bengaluru. PhonePe was founded in December 2015, by Sameer Nigam, Rahul Chari and Burzin Engineer. The PhonePe app, based on the Unified Payments Interface (UPI), went live on August 2016. It is currently owned by Flipkart.")
    col1,col2, = st.columns(2)
    with col1:
        st.image(phn)
    with col2:
...
if SELECT == "About":
    st.subheader("PhonePe Pulse is a destination for accurate and granular data on digital payments."
                 "The PhonePe Pulse platform showcases data from more than 2,000 crore transactions by digital payment consumers in India, starting 2018."
                 "PhonePe has been a strong beneficiary of the API driven digitisation of payments in India. PhonePe has been a strong beneficiary of the API driven digitisation of payments in India "
                 "PhonePe Pulse is a way of giving back to the digital payments ecosystem.")
    col1,col2 = st.columns(2)
    with col1:
        st.image(phn1)
        st.download_button("DOWNLOAD THE APP NOW", "https://www.phonepe.com/app-download/")
...
if SELECT == "Contact":
    name = " N T SRIVATHSAN "
    mail = (f'{"Mail :"}  {"srivathsan29oct@gmail.com"}')
    social_media = {"GITHUB": "https://github.com/Srivathsan221",
                    "LINKEDIN": "https://www.linkedin.com/in/srivathsanthiruvenkatachari/"
                    }
    col1,  col2 = st.columns(2)
    with col1:
        st.image(mypic)
    with col2:
        st.title(name)
        st.subheader("An aspiring person to become a Data Analyst!")
        st.write("---")
        st.subheader(mail)
    #st.write("#")
